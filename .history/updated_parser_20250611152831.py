...
# class Protein:
#     """
#     Parses .pdb file to extract residue indices, type encodings, Ca coords,
#     Carbon backbone edges, spatial proximity edges for graph construction
#     """
...
# class Ligand:
#     """
#     Parses .mol2 (or SDF) ligand files to extract atomic numbers, 3D coords, charges, 
#     hybridization, aromaticity, ring memberships, topological descriptors, bond indices and types
#     """
...
class Protein:
    ...
    def _compute_edges(self):
        coords = np.vstack(self.pos_CA)  # [N,3]
        n = coords.shape[0]
        ...
        # Spatial proximity edges (type=1)
        # Use upper triangle to avoid duplicates and self-edges
        tri_upper = np.triu_indices(n, k=1)
        mask = d2[tri_upper] <= self.threshold * self.threshold
        prox_i = tri_upper[0][mask]
        prox_j = tri_upper[1][mask]
        prox_dists = np.sqrt(d2[prox_i, prox_j])
        prox_types = np.ones(len(prox_dists), dtype=np.int64)

        # Combine edges
        self.edges = np.hstack([
            np.vstack([backbone_i, backbone_j]),
            np.vstack([prox_i, prox_j])
        ])
        self.edge_distances = np.concatenate([backbone_dists, prox_dists])
        self.edge_types = np.concatenate([backbone_types, prox_types])
...
ppath = '/Users/vedantajain/ConiferPoint/DeNovo3D/6ugn_pocket.pdb'
lpath = '/Users/vedantajain/ConiferPoint/DeNovo3D/6ugn_ligand.mol2'

prot = Protein(ppath).to_dict()
lig = Ligand(lpath).to_dict()

print("Protein features:", prot)
print("Ligand features:", lig)