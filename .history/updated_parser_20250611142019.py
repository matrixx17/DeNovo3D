...
class Protein:
    ...
    def compute_edges(self):
        ...
        tree = KDTree(coords)
        pairs = tree.query_pairs(r=self.threshold) 
        if not pairs:
            raise ValueError("No edges within threshold")
        i, j = zip(*pairs)
        self.edges = np.vstack([i,j])
        dists = np.linalg.norm(coords[i] - coords[j], axis=1)
        self.edge_dists = dists
    
    def finalize(self):
        self.residues = np.array(self.residues, dtype=np.int64)
        self.CA_pos = np.array(self.CA_pos, dtype=np.float32)
